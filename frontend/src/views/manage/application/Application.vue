            ...
            <a-col :md="6" :sm="24">
              <a-form-item
                ...
                <a-select v-model="queryParams.status" allowClear>
                  <a-select-option value="待初审">待初审</a-select-option>
                  <a-select-option value="待终审">待终审</a-select-option>
...
import dishesView from './ApplicationView.vue'
...
export default {
  name: 'dishes',
  components: { dishesView, RangeDate},
  data () {
    return {
      advanced: false,
      dishesAdd: {
  ...
  methods: {
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.$delete('/cos/application-info/' + ids).then(() => {
    ...
    fetch (params = {}) {
      ...
      this.$get('/cos/application-info/page', {
        ...params
      }).then((r) => {
